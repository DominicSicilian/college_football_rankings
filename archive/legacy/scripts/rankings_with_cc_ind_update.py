...
def is_independent(conference_name):
    """Check if a conference name indicates an independent team"""
    return conference_name.lower() in ["independent", "fbs independents", "ind"]
...
class My_Conf:
    def __init__(
        ...
        conf_index=None,
        cvc_perc=None,
        is_individual_independent=False,
    ):
        self.name = name
        self.p5 = p5
        ...
        self.is_individual_independent = (
            is_individual_independent  # Flag for individual independent teams
        )
...
class My_Team:
    def __init__(
        ...
        individual_conf_name=None,
    ):
        self.name = name
        self.conf_name = conf_name
        self.individual_conf_name = individual_conf_name  # For independents, this will be their individual "conference"
        self.wins = wins
        self.losses = losses
        ...
        self.SPI = SPI
        self.cross_conf_value = cross_conf_value
...
def conference_rankings(year):
    ...
    try:
        ...
        for i_it, team in enumerate(teams):
            ...
            # For independents, create individual "conferences"
            if is_independent(conference):
                individual_conf_name = (
                    f"independent_{team.school.lower().replace(' ', '_')}"
                )
                is_p5 = False  # Independents are not P5
            else:
                individual_conf_name = conference
                is_p5 = conference in power5

            # Use individual conference name for independents
            conf_key = (
                individual_conf_name if is_independent(conference) else conference
            )

            if conf_key not in conf_objects:
                this_conf = My_Conf(
                    name=conf_key,
                    p5=is_p5,
                    is_individual_independent=is_independent(conference),
                )
                conf_objects.update({conf_key: this_conf})

            # Get team records directly from games
            wins, losses = get_team_record(team.school, year)

            # For independents, conference record is 0-0 since they don't play conference games
            if is_independent(conference):
            ...
            OOC_W = wins if is_independent(conference) else wins - conf_wins
            OOC_L = losses if is_independent(conference) else losses - conf_losses

            conf_objects[conf_key].cvc_wins += OOC_W
            conf_objects[conf_key].cvc_losses += OOC_L
            ...
            conf_objects[conf_key].cvc_perc = OOC_perc
...
def total_conference_rankings():
    ...
    for conference in LY_confs:
        ...
        this_conf = My_Conf(
            ...
            is_individual_independent=that_conf.is_individual_independent,
        )
    ...
    for i_it, conference in enumerate(TY_confs):
        ...
        this_conf.is_individual_independent = that_conf.is_individual_independent
    ...
    # Sort conferences: P5 first, then by win percentage, then by wins, then by fewer losses
    # Individual independents will be sorted by their own win percentage
...
def display_and_save(SPI_final_rankings, conf_rankings):
    ...
    for i_it, tm_obj in enumerate(SPI_final_rankings):
        ...
        for j_it, conf_obj in enumerate(conf_rankings):
            c_rk = j_it + 1
            # For independents, match by individual conference name
            if hasattr(tm_obj, "individual_conf_name") and tm_obj.individual_conf_name:
                if conf_obj.name == tm_obj.individual_conf_name:
                    conf_ranking = c_rk
                    break
            elif conf_obj.name == conf_name:
                conf_ranking = c_rk
                break
...
def display_cross_conference_standings(cross_conf_rankings):
    ...
    cross_conf_file = os.path.join(
        "data_exports", f"cross_conference_standings_{today_year}.csv"
    )
...
def main():
    ...
    try:
        ...
        for i_it, team in enumerate(teams):
            ...
            # For independents, create individual conference names
            if is_independent(conference):
                individual_conf_name = (
                    f"independent_{team.school.lower().replace(' ', '_')}"
                )
                conf_key = individual_conf_name
            else:
                individual_conf_name = conference
                conf_key = conference

            if conf_key not in team_objects:
                team_objects.update({conf_key: []})
            ...
            team_obj = My_Team(
                ...
                ave_margin=A,
                points=P,
                individual_conf_name=individual_conf_name,
            )

            # Store team object
            team_objects[conf_key].append(team_obj)
